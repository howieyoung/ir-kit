...
4. **Docs stay generated.** Tutorial content lives in `public/js/content/guide.<locale>.js`; run `npm run build-tutorial` after editing guide content in any locale (CI fails on stale tutorials — it regenerates all six). Translated READMEs live in `docs/i18n/` — substantive README changes should be reflected there too.
